...
function Dropdown({
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-4 px-4 py-2.5 bg-black/[0.03] rounded-xl font-bold text-sm text-[#1A1A1A] hover:bg-black/[0.05] transition-all"
      >
        <span className="truncate max-w-[120px] sm:max-w-none">{value}</span>
        <ChevronDown
          size={16}
          className={cn('text-primary transition-transform duration-200 shrink-0', open && 'rotate-180')}
        />
      </button>
      <AnimatePresence>
        {open && (
  ...
  return (
    <div className="min-h-screen bg-[#E5E5E5] pt-28 md:pt-32 pb-20">
      <div className="container mx-auto px-4 md:px-6 max-w-7xl">
        ...
        <h1 className="text-[#1A1A1A] font-bold text-3xl md:text-5xl text-center mb-8 md:mb-12">
          Find Your Medium
        </h1>
        ...
        <div className="max-w-3xl mx-auto relative mb-6 md:mb-8 group">
          ...
          <input
            ...
            className="w-full h-14 md:h-16 pl-12 md:pl-14 pr-12 bg-[#F8F8F8] rounded-2xl border border-white/50 shadow-sm outline-none focus:ring-2 focus:ring-primary/10 transition-all font-medium text-[#1A1A1A]"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
        ...
        <div className="bg-[#F8F8F8] rounded-[24px] p-5 md:p-6 md:px-8 border border-white/50 shadow-sm mb-8 md:mb-10">
          <div className="flex items-center gap-2 mb-4 md:mb-0 md:hidden">
            <Filter size={16} className="text-primary" />
            <span className="font-bold text-sm text-[#1A1A1A]">Filters</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:flex-wrap md:items-center gap-4 md:gap-8 lg:gap-12">
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm font-bold text-[#666666] shrink-0">Specialty:</span>
              <Dropdown options={SPECIALTIES} value={specialty} onChange={setSpecialty} />
            </div>

            {/* Sort By */}
            <div className="flex items-center gap-3">
              <span className="text-sm font-bold text-[#666666] shrink-0">Sort By:</span>
            ...
            <div className="flex items-center gap-3 md:flex-grow md:max-w-[300px]">
              <span className="text-sm font-bold text-[#666666] shrink-0">Price:</span>
              <div className="flex-grow flex items-center gap-3">
                ...
                <Slider
                  ...
                  className="flex-grow"
                />
                <span className="text-xs font-bold text-[#1A1A1A]/40 whitespace-nowrap">${priceRange[0]}/min</span>
              </div>
            </div>
            ...
            <button
              onClick={clearAll}
              className={cn(
                'text-sm font-bold transition-colors underline underline-offset-4 text-left md:ml-auto',
        ...
        {/* Results Info */}
        <p className="text-[#666666] font-bold text-sm mb-6 md:mb-8">
            ...
            <motion.div key="grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {filtered.map((medium, index) => (
                <motion.div
                  ...
                  className="bg-[#F8F8F8] rounded-[24px] md:rounded-[32px] p-5 md:p-6 border border-white/50 shadow-sm hover:shadow-xl transition-all group"
                >
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                    ...
                    <div className="flex-grow space-y-2 sm:space-y-3">
                      <div className="flex justify-between items-start">
                        <h3 className="font-bold text-lg sm:text-xl text-[#1A1A1A] leading-tight">{medium.name}</h3>
                        <div className={cn(
                          'px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shrink-0 ml-2',
                  ...
                  <div className="mt-5 pt-5 border-t border-black/5 flex items-center justify-between">
                    ...
                    <Link
                      ...
                      className="bg-primary hover:bg-primary/90 text-white font-bold px-5 md:px-6 py-2.5 rounded-xl text-sm transition-all active:scale-95 shadow-lg shadow-primary/20"
                    >
                      View Profile
                    </Link>
                  </div>
